...
class RealWorldEngine @Inject constructor() {
    ...
    fun frameAhead(
        ...
        val atmosphere = RealWorldAtmosphere(
            sky = sky,
            roadFeel = feel,
            routePulseColor = pulseColor(sky, feel),
    ...
    private fun MutableList<RealWorldCue>.addRoadFeel(
        ...
        add(
            cue(
                ...
                icon = "▱",
                color = pulseColor(environment.sky(), feel),
                current = current,
                priority = 50,
...
private fun pulseColor(sky: RealWorldSky, feel: RealWorldRoadFeel): String = when {
    sky == RealWorldSky.RAIN -> "#38BDF8"
    sky == RealWorldSky.SNOW || sky == RealWorldSky.FOG -> "#CBD5E1"
