...
class GraphHopperEngine @Inject constructor(
    ...
    fun load() {
        ...
        hopper = GraphHopper().apply {
            ...
            setProfiles(
                Profile("car").apply { vehicle = "car"; weighting = "fastest" },
                Profile("car_short").apply { vehicle = "car"; weighting = "shortest" },
            )
            ...
            try {
                ...
                val routes = response.all.mapIndexed { index, path ->
                    Route(
                        ...
                        geometry = path.points.toNavPlusLatLngs(),
                        steps = path.instructions.map { instr ->
                            RouteStep(
                                ...
                                startLocation = LatLng(
                                    path.points.getLat(instr.firstNode),
                                    path.points.getLon(instr.firstNode),
                                ),
                                endLocation = LatLng(
                                    path.points.getLat(instr.lastNode.coerceAtMost(path.points.size - 1)),
                                    path.points.getLon(instr.lastNode.coerceAtMost(path.points.size - 1)),
                                ),
                                geometry = path.points
                                    .toNavPlusLatLngs()
                                    .subList(
                                        instr.firstNode.coerceIn(0, path.points.size - 1),
                                        (instr.lastNode + 1).coerceIn(0, path.points.size),
                                    ),
